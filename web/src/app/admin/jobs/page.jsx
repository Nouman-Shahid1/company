...
import { deleteCategory } from "../../reducers/Category/categorySlice";
import CategoryModal from "../../components/Modal/Admin/Category/CreateOrUpdate";
...
const ListCategories = () => {
  ...
  const handleSearchCategories = (e) => {
    if (e.key === "Enter") {
      dispatch(
        fetchCategories({ limit: LIMIT, offset: 0, search: searchText })
      );
    }
  };

  const handlePageChange = (selectedPage) => {
    dispatch(fetchCategories({ limit: LIMIT, offset: LIMIT * selectedPage }));
  };

  const handleBulkDelete = (e) => {
    e.preventDefault();

    dispatch(bulkDeleteCategory(selectedCategories.map((cat) => cat.id)));
    setBulkDeleteModal(false);
    setSelectedCategories([]);
  };

  useEffect(() => {
    dispatch(fetchCategories({ limit: LIMIT, offset: 0 }));
  }, [dispatch]);
                ...
                <div className="flex items-center rounded-lg gap">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onKeyDown={handleSearchCategories}
                    />
                    <button
                      type="submit"
