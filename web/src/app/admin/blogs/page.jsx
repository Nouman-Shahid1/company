...
import {
  ...
  deleteBlog,
  bulkDeleteBlogs,
} from "../../../reducers/Blog/blogSlice"; // Update these imports based on your actual blog slice
 // Update based on your actual component
import Profile from "../../../components/Profile/Profile"; // Update based on your actual component
import DeleteIcon from "../../../assets/images/icons/deleteIcon.png"; // Update based on your actual asset
import DummyIcon from "../../../assets/images/dummyIcon.png"; // Update based on your actual asset
...
export default function Blogs() {
  ...
  const handlePageChange = (selectedPage) => {
    dispatch(fetchBlogs({ limit: LIMIT, offset: LIMIT * selectedPage }));
  };

  const handleSearchBlogs = (e) => {
    if (e.key === "Enter") {
      dispatch(fetchBlogs({ limit: LIMIT, offset: 0, search: searchText }));
    }
  };

  const handleCheckboxChange = (e, blog) => {
    if (e.target.checked) {
  ...
  const handleBulkDelete = (e) => {
    e.preventDefault();
    dispatch(bulkDeleteBlogs(selectedBlogs.map((b) => b.id)));
    setBulkDeleteModal(false);
    setSelectedBlogs([]);
  };

  useEffect(() => {
    dispatch(fetchBlogs({ limit: LIMIT, offset: 0 }));
  }, [dispatch]);
        ...
        <div className="pt-[30px] bg-white flex flex-col overflow-x-auto mt-8 rounded-3xl h-full">
          ...
          <table className="min-w-full mt-5 w-full">
            <thead className="bg-[#FAFBFF]">
              <th
                scope="col"
                className="py-3.5 px-4 text-left rtl:text-right dark:text-gray-400 font-poppinsmedium font-medium text-lg text-[#B5B7C0]"
              ></th>
              <th
                scope="col"
                className="py-3.5 px-4 text-left rtl:text-right dark:text-gray-400 font-poppinsmedium font-medium text-lg text-[#B5B7C0]"
              ...
              <th
                scope="col"
                className="py-3.5 px-4 font-poppinsmedium font-medium text-lg text-[#B5B7C0] text-left rtl:text-right dark:text-gray-400"
              >
                Slug
              </th>
              ...
              <th
                scope="col"
                className="py-3.5 px-4 text-center rtl:text-right dark:text-gray-400 font-poppinsmedium font-medium text-lg text-[#B5B7C0]"
              >
                Actions
              </th>
            </thead>
            <tbody className="bg-white divide-y w-full divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
              {blogs.length > 0 ? (
                blogs.map((blog, index) => (
                  <tr key={blog.id}>
                    <td className="pl-2 md:pl-8">
                      <label key={index} className="relative">
                        <input
                          type="checkbox"
                          className="checkbox"
                          onChange={(e) => handleCheckboxChange(e, blog)}
                          checked={
                            !!selectedBlogs.find((b) => b.id === blog.id)
                          }
                        />
                      </label>
                    </td>
