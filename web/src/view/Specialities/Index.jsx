...
import {
  fetchSpecialities,
  bulkDeleteSpecialities,
  deleteSpeciality,
  specialitySlice,
} from "../../reducers/Speciality/specialitySlice";
...
import SpecialityModal from "../../components/Modal/Admin/Speciality/CreateOrUpdate";
...
const ListSpecialities = () => {
  ...
  const { specialities, count } = useSelector((state) => state.speciality);
  ...
  const handleCheckboxChange = (e, speciality) => {
    if (e.target.checked) {
      setSelectedSpecialities([...selectedSpecialities, speciality]);
    } else {
      setSelectedSpecialities(
        selectedSpecialities.filter((spec) => spec.id !== speciality.id)
      );
    }
  };
  ...
  const handleBulkDelete = (e) => {
    ...
    dispatch(
      bulkDeleteSpecialities(selectedSpecialities.map((spec) => spec.id))
    );
    setBulkDeleteModal(false);
    setSelectedSpecialities([]);
  };
  const handlePageChange = (selectedPage) => {
    dispatch(fetchSpecialities({ limit: LIMIT, offset: LIMIT * selectedPage }));
  };

  const handleSearchSpecialities = (e) => {
    if (e.key === "Enter") {
      dispatch(
        fetchSpecialities({ limit: LIMIT, offset: 0, search: searchText })
      );
    }
  };
  ...
  useEffect(() => {
    dispatch(fetchSpecialities({ limit: LIMIT, offset: 0 }));
  }, [dispatch]);
                ...
                <div className="flex items-center rounded-lg gap ">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onKeyDown={handleSearchSpecialities}
                    />
                    <button
                      type="submit"
                  ...
                  <tr key={speciality.id}>
                    <td className="pl-2 md:pl-4 ">
                      <label key={index} className="relative">
                        <input
                          type="checkbox"
                          className="checkbox"
                          onChange={(e) => handleCheckboxChange(e, speciality)}
                          checked={
                            !!selectedSpecialities.find(
                              (spec) => spec.id === speciality.id
                            )
                          }
                        />
                      </label>
                    ...
                    <td className="py-3.5 px-4 md:px-4 text-sm font-medium text-gray-900 dark:text-gray-300">
                      <div className="flex items-center justify-center gap-x-2">
                        <SpecialityModal speciality={speciality} />
                        <DeleteModal
                          itemId={speciality.id}
