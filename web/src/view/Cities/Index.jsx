...
import { bulkDeleteCities, fetchCities } from "../../reducers/City/citySlice";
import { deleteCity } from "../../reducers/City/citySlice";
import CityModal from "../../components/Modal/Admin/City/CreateOrUpdate";
...
import DummyIcon from "../../assets/images/dummyIcon.png";

export default function ListCities() {
  const LIMIT = 10;

  const dispatch = useDispatch();
  const { cities, count } = useSelector((state) => state.city);
  ...
  const handlePageChange = (selectedPage) => {
    dispatch(fetchCities({ limit: LIMIT, offset: LIMIT * selectedPage }));
  };

  const handleSearchCities = (e) => {
    if (e.key === "Enter") {
      dispatch(fetchCities({ limit: LIMIT, offset: 0, search: searchText }));
    }
  };
  const handleCheckboxChange = (e, city) => {
    if (e.target.checked) {
      ...
      setSelectedCities(selectedCities.filter((cit) => cit.id !== city.id));
    }
  };
  ...
  const handleBulkDelete = (e) => {
    ...
    dispatch(bulkDeleteCities(selectedCities.map((cit) => cit.id)));
    setBulkDeleteModal(false);
    setSelectedCities([]);
  };
  useEffect(() => {
    dispatch(fetchCities({ limit: LIMIT, offset: 0 }));
  }, [dispatch]);
                    ...
                    <td className="pl-2 md:pl-8">
                      <label key={index} className="relative">
                        <input
                          ...
                          checked={
                            !!selectedCities.find((cit) => cit.id === city.id)
                          }
                        />
                      </label>
                    </td>
